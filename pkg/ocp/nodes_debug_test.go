...
func TestNodesDebugExecCreatesPrivilegedChrootPod(t *testing.T) {
	...
	out, err := NodesDebugExec(context.Background(), nil, "", "worker-0", "", []string{"uname", "-a"}, 2*time.Minute)
	if err != nil {
...
func TestNodesDebugExecReturnsErrorForNonZeroExit(t *testing.T) {
	...
	out, err := NodesDebugExec(context.Background(), nil, "debug-ns", "infra-node", "registry.example/custom:latest", []string{"journalctl", "-xe"}, time.Minute)
	if err == nil {
...
func TestCreateDebugPod(t *testing.T) {
	_ = NewNodeDebugTestEnv(t)

	created, err := createDebugPod(context.Background(), nil, "worker-1", "test-ns", "custom:v1", []string{"ls", "-la"})
	if err != nil {
...
func TestPollForCompletion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			created, _ := createDebugPod(context.Background(), nil, "node-1", "default", DefaultNodeDebugImage, []string{"echo", "test"})

			terminated, lastPod, waitMsg, err := pollForCompletion(context.Background(), env.Pods, created.Name, time.Minute)
...
func TestRetrieveLogs(t *testing.T) {
	...
	logs, err := retrieveLogs(context.Background(), env.Pods, created.Name)
	if err != nil {
